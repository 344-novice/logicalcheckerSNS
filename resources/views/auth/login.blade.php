<x-guest-layout>
    <div class="flex justify-between items-end mb-4 text-gray-600 dark:text-gray-400">
        ...
        <a class="underline text-sm text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-gray-100"
            href="{{ route('register') }}"
            >
            {{ __('アカウントの新規作成はこちら') }}
        </a>
    ...
    <form id="login-form" method="POST" action="{{ route('login') }}" data-dialup="true">
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox" name="remember" class="form-checkbox checked:bg-blue-500 checked:ring-0 focus:ring-0 border-2 focus:outline-none rounded">
        ...
        <div class="flex items-center justify-end mt-4">
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 hover:font-bold">
                    {{ __('パスワードを忘れた場合はこちら') }}
                </a>
            @endif
